...
pub trait Stage: Send + Sync + fmt::Debug {
    ...
    fn accepts(&self) -> Option<&[MediaType]> {
        None
    }
}
...
impl Pipeline {
    ...
    pub async fn process_batch(&self, bufs: Vec<MediaBuffer>) -> Result<Vec<MediaBuffer>> {
        let mut out = Vec::new();
        for buf in bufs {
            out.extend(self.process(buf).await?);
        }
        Ok(out)
    }
}
...
impl PipelineBuilder {
    ...
    #[must_use]
    pub fn stage(mut self, s: impl Stage + 'static) -> Self {
        self.stages.push(Arc::new(s));
        self
    }
    #[must_use]
    pub fn stage_arc(mut self, s: Arc<dyn Stage>) -> Self {
        self.stages.push(s);
        self
    }
...
mod tests {
    ...
    fn buf() -> MediaBuffer {
        MediaBuffer::builder(MediaType::Video)
            .data(vec![0u8; 32])
            .build()
            .unwrap()
    }
